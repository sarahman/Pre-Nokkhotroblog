 ...
 */
class Blog_Model_Dao_BlogGroupPost extends Speed_Model_Dao_Abstract
{
    ...
    public function getDetail($groupId)
    {
        $select = $this->select()
            ...
            ->where("{$this->_name}.blog_group_id=?", $groupId)
            ->where("{$this->_name}.blog_group_post_is_published=?", 1)
            ->order(array("{$this->_primaryKey} DESC"));
    ...
    public function getDetailByPermalink($permalink)
    {
        $select = $this->select()
            ...
            ->join('blog_groups', "blog_groups.blog_group_id = {$this->_name}.blog_group_id")
            ->join('users', "{$this->_name}.create_by = users.user_id")
            ->where("{$this->_name}.blog_group_post_permalink=?", $permalink)
            ->where("{$this->_name}.blog_group_post_is_published=?", 1)
            ->order(array("{$this->_primaryKey} DESC"));
        return $this->returnResultAsAnArray($this->fetchRow($select));
    }
